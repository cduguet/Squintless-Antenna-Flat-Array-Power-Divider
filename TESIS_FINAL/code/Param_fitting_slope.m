...
%dentro en b.
for ibr=1:4; %br=3.5,4.5,5.5,6.5
for ib1=1:10; %b1=1,...,10
for ib=1:10; %b=1,...,10
    S_generic(ib1,ib,ibr,:)=data(:,800+(ibr-1)*100 + (ib1-1)*10 +ib);
end
end
end
...
for ib1=1:10; %b1=1,...,10
for ib=1:10; %b=1,...,10
    S_ins(ib1,ib,ibr,:)=data(:,800+(ibr-1)*100 + (ib1-1)*10 +ib);
end
end
end
%-------------------------------------------------------------------------%
...
for ibr = 1:4
    ...
    fprintf('b1 \t & \t b (S max) \t & \t Variación máxima(\\%%) \t & \t Variación en b=b1+br (\\%%) \\\\ \\hline \n');
    for ib1=1:10
...
figure(5)
[a,b] = ndgrid(1:10,1:4);
data=squeeze(S_generic(:,8,:,16));
surface(a,b,data); title('Variación del acoplamiento, con br');
xlabel('b1');ylabel('br');
...
for ibr =1:4 %for br = 1.5mm,...,6.5mm
    for ib1 = 1:10 %for b1 =1mm,...,10mm
        S_generic_restr(ib1,ibr,:) = S_generic(ib1,min(ib1+ibr+1,10),ibr,:);
    end
end
...
for ibr =1:4 %for br = 1.5mm,...,6.5mm
    for ib1 = 1:10 %for b1 =1mm,...,10mm
        S_ins_restr(ib1,ibr,:) = S_generic(ib1,min(ib1+ibr+1,10),ibr,:);
    end
end
...
%ndgrid
[xdata.b1,xdata.br,xdata.f] =   ndgrid(1:10, 3.5:1:6.5, 8.5:.1:11.5);
