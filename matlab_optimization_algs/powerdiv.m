...
P=A.^2;
Ptot=sum(P);
b=zeros(size(A)); %se inicializa vector de alturas de guía
...
b(1)= 12; %mm (altura de la guía en el primer escalón)

%límites de br
brmin=P(1)*b(1)/(Ptot - P(1)) - P(N)*b(1)/(Ptot-P(1))
brmax=P(1)*b(1)/(Ptot - P(1))
br = 1.66 %mm (altura de la guía que alimenta a cada radiador)
%1.672
